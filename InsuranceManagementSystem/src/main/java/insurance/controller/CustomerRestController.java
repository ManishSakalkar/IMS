...
import insurance.util.CustomerUtil;
...
public class CustomerRestController {
	...
	/**
	 * 1. Read JSON(Customer) and convert to Object Format
			...
			@RequestBody Customer customers)
	{
		...
		try {
			...
			Integer id = service.saveCustomer(customers);
			...
			)
	{
		...
		try {
			...
			if(opt.isPresent()) {
				...
				util.mapToActualObject(actual,customers);
